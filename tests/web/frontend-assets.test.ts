...
describe('web frontend source', () => {
  ...
  it('renders message and attachment download status labels', () => {
    ...
    expect(i18n).toContain('Downloaded')
    expect(i18n).toContain('Partially downloaded')
    expect(i18n).toContain('Not downloaded')
    expect(app).toContain('downloadStateLabel(state, t)')
  ...
  it('keeps guard workbench aligned with guard API payloads', () => {
    ...
    expect(app).toContain('{!guardOnly ? (')
    expect(app).not.toContain('className="view-tabs"')
    expect(app).not.toContain('setView')
