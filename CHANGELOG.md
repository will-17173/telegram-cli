...
- Refresh Telegram send peers after `CHANNEL_INVALID` so interactive `tg listen` group sends can recover before any runtime message peer is observed.

## [0.8.5] - 2026-07-29

### Fixed

- Show attachment summaries in stored message tables, including media-only `tg recent --chat --user` results.
...
- Refresh Telegram history peers and extend transient retries after repeated `CHANNEL_INVALID` pagination failures during `tg sync`.
