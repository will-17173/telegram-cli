...
function score(definition: ListenCommandDefinition, query: readonly string[]): number {
  ...
  const joinedQuery = normalize(query.join(' '))
  const fields = [
    path.join(''),
    definition.summary,
    ...definition.keywords,
  ...
  const tokenwisePath = query.length <= path.length
    && query.every((part, index) => isSubsequence(normalize(part), normalize(path[index] ?? '')))
...
function parseQuery(input: string): ParsedQuery | undefined {
  ...
  if (tokens.length === 0) return { tokens, query: [] }
  const values = tokens.map(token => token.value.toLowerCase())
  if (values[0] === '') return { tokens, query: [] }
  ...
  const query = tokens.length > 1 && anyMatch(two) ? two : values.slice(0, 1)
  return { tokens, query }
}
