...
import { resolveAccountContext, resolveAuthenticatedAccountContext } from '../account/account-context.js'
...
export async function runWithAuthenticatedAccountContext<T>(
  options: AccountCommandOptions,
  handler: (context: AccountContext) => Promise<HandlerResult<T> | void> | (HandlerResult<T> | void),
  command?: Command,
): Promise<void> {
  ...
  let context: AccountContext
  try {
    context = resolveAuthenticatedAccountContext({
      explicitName: effectiveOptions.account,
      dataDir: getDataDir(),
    })
  } catch (error) {
...
export function accountFailureFromError(error: unknown): HandlerResult<never> {
  ...
  if (code === 'account_required' || code === 'account_not_found' || code === 'account_logged_out' || code === 'account_session_missing') {
    return {
      ok: false,
      error: {
        code,
