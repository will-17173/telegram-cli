...
export class MtcuteGroupTopics {
  ...
  async createTopic(r: W.TelegramCreateTopicRequest) { const x = await this.group(r.chat); try { const message = await this.client.createForumTopic({ chatId: x.id, title: r.title, icon: r.iconEmojiId == null ? r.iconColor : Long.fromString(r.iconEmojiId) }); const [topic] = await this.client.getForumTopicsById(x.id, [message.id]); if (!topic) throw new Error('Created topic was not returned by Telegram'); return { chat_id: x.group.id, topic: mapTopic(topic) } } catch (e) { throwWriteError(e, r.chat) } }
  async editTopic(r: W.TelegramEditTopicRequest) { const x = await this.group(r.chat); try { await this.client.editForumTopic({ chatId: x.id, topicId: r.topicId, title: r.title, icon: r.iconEmojiId == null ? r.iconEmojiId : Long.fromString(r.iconEmojiId) }); const [topic] = await this.client.getForumTopicsById(x.id, [r.topicId]); if (!topic) throw new Error('Edited topic was not returned by Telegram'); return { chat_id: x.group.id, topic: mapTopic(topic) } } catch (e) { throwWriteError(e, r.chat) } }
